...
- **Recent user interactions:** `keepRecentUserTurns` (default `0`) excludes the latest N user interactions, including the current one, from all condense rewrites. A shared positional boundary limits capture and compression to older work and runs the render pipeline only on the eligible prefix, preserving the protected suffix by reference. No extra persisted state; branch rescans make older interactions eligible. Pressure and manual compaction cannot override it. This does not undo historical loss or govern native Pi compaction. See [configuration](doc/configuration.md#recent-user-interactions).
